...
if st.button("Estimate price", type="primary"):
    ...
    if len(window) < 5:
        ...
        if matched_band is None:
            ...
            st.info(f"Your estimate of S${price:,.0f} sits in a price band with no "
                    f"recorded {flat_type} sales, so it is not highlighted, but it "
                    f"falls within the overall range of past prices")
        else:
